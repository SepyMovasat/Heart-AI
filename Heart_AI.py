...
class ML_Model():
    ...
    def gather_data():
        ...
        try:
            ...
            if simple:
                bar = UI.ProgressBar(5,"Gathering Data")
            else:
...
Input your resting electrocardiographic results{UI.red_text('(JUST THE NUMBER)')}: """))
    ...
    def Train_model(simple=False):
        ...
        if not simple:
            ...
            features = ['age', 'sex', 'cp', 'thalachh', 'exng']
            target = 'output'
...
""")
        ...
        match inp:
            case 1:
                ...
                try:
                    UI.cls()
                    print(UI.green_text("<-----------Mode Selection----------->"))
                    inp2 = int(input(f"""{UI.cyan_text('Modes')}:
{UI.magenta_text('1- Simple')} - 70% accurate, doesn't need complicated information
{UI.magenta_text('2- Default')} - 85% accurate, needs complicated information
Select the mode{UI.red_text("(Just enter the number)")}: """))
                except ValueError as e:
                    UI.Info_Style()
                    print("Please do not enter anything more than numbers")
                    UI.Error_Style()
                ...
                ML_Model.Train_model(simple=simple)
                ...
                if result:
                    ...
                    if not simple:
                        ...
                        print(f"{UI.red_text('NOTE:')} Your are using the {UI.cyan_text('simple mode')} which is about {UI.green_text('70% accurate')}")
